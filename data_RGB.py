...
class test_dataset:
    def __init__(self, image_root, gt_root, d_root, testsize):
        self.testsize = testsize
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.d = [d_root + f for f in os.listdir(d_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.jpg')
                       or f.endswith('.png')]
    ...
    def load_data(self):
        ...
        d = self.binary_loader(self.d[self.index])
        d = self.depth_transform(d).unsqueeze(0)
        d = torch.div(d.float(),255.0)
        name = self.images[self.index].split('\\')[-1]
        if name.endswith('.jpg'):
            name = name.split('.jpg')[0] + '.png'
        self.index += 1
        return image, gt, d, name
