...
class vgg16_depth_b_zacn(nn.Module):
    def __init__(self, channel=32):
        super(vgg16_depth_b_zacn, self).__init__()
    ...
    def forward(self, x, depth):
        ...
        depth = F.interpolate(depth, h.size()[2:], mode='bilinear', align_corners=True)
        depth = depth + torch.randn_like(depth) *0.000001
        offset = computeOffset(depth[:, 0], 3, 1)
        offset = F.pad(offset, (1,1,1,1), "constant", 0).contiguous()

        dg = self.bn5_3b(self.conv5_3b(self.bn5_2b(self.conv5_2b(self.bn5_1b(self.conv5_1b(h)), offset))))

        h = self.relu5_1(self.bn5_1(self.conv5_1(h)))
        h = self.relu5_2(self.bn5_2(self.conv5_2(h)))
        h = self.relu5_3(self.bn5_3(self.conv5_3(h)) + dg)
...
class vgg16_depth_b(nn.Module):
    def __init__(self, channel=32):
        super(vgg16_depth_b, self).__init__()
        ...
        self.conv5_1b = nn.Conv2d(512, 64, kernel_size=1, stride=1, bias=False)
        self.bn5_1b = nn.BatchNorm2d(64, eps=1e-05, momentum=0.1, affine=True)
        self.conv5_2b = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn5_2b = nn.BatchNorm2d(64, eps=1e-05, momentum=0.1, affine=True)
        self.conv5_3b = nn.Conv2d(64, 512, kernel_size=1, stride=1, bias=False)
        self.bn5_3b = nn.BatchNorm2d(512, eps=1e-05, momentum=0.1, affine=True)
    ...
    def forward(self, x):
        ...
        dg = self.bn5_3b(self.conv5_3b(self.bn5_2b(self.conv5_2b(self.bn5_1b(self.conv5_1b(h))))))
...
class vgg16_depth_zacn(nn.Module):
    def __init__(self, channel=32):
        super(vgg16_depth_zacn, self).__init__()
    ...
    def forward(self, x, depth):
        ...
        depth = F.interpolate(depth, h.size()[2:], mode='bilinear', align_corners=True)
        depth = depth + torch.abs(torch.randn_like(depth) *0.0000001)
        offset = computeOffset(depth[:, 0], 3, 1)
        offset = F.pad(offset, (1,1,1,1), "constant", 0).contiguous()
        dg = self.bn5_3b(self.conv5_3b(self.bn5_2b(self.conv5_2b(self.bn5_1b(self.conv5_1b(h)), offset))))
